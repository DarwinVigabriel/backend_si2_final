...
class Labor(models.Model):
    ...
    campaña = models.ForeignKey(
        ...
        blank=True,
        null=True,
        help_text='Campaña a la que pertenece esta labor (opcional)'
    )
    parcela = models.ForeignKey(
        Parcela,
        on_delete=models.SET_NULL,
    ...
    def clean(self):
        ...
        if not self.campaña and not self.parcela:
            raise ValidationError('Debe especificar al menos una campaña o una parcela.')

        # Validar que la fecha esté dentro del rango de la campaña, si se especifica campaña
        if self.campaña and self.fecha_labor:
            if self.fecha_labor < self.campaña.fecha_inicio or self.fecha_labor > self.campaña.fecha_fin:
                raise ValidationError({
                    'fecha_labor': f'La fecha de la labor debe estar dentro del rango de la campaña: {self.campaña.fecha_inicio} a {self.campaña.fecha_fin}'
                })
...
class ProductoCosechado(models.Model):
    ...
    - Relación con campaña O parcela (una de las dos)
    ...
    campania = models.ForeignKey(
        ...
        null=True,
        related_name='productos_cosechados',
        help_text='Campaña a la que pertenece el producto (opcional)'
    )
    ...
    def __str__(self):
        ...
        opcion_str = ""
        if self.campania:
            opcion_str = f" - Campaña: {self.campania.nombre}"
        elif self.parcela:
    ...
    def clean(self):
        ...
        if self.campania and self.labor.campania != self.campania:
            raise ValidationError({
                'labor': f'La labor seleccionada no pertenece a la campaña {self.campania.nombre}.'
            })
    ...
    def origen_display(self):
        """Retorna el origen (campaña o parcela) en formato legible"""
        if self.campania:
            return f"Campaña: {self.campania.nombre}"
        elif self.parcela:
