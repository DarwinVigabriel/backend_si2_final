"""
CU11: MÓDULO DE REPORTES PARA campaniaS
...
class CampaignReports:
    """
    Clase para generar reportes relacionados con campanias agrícolas
    """
    ...
    def get_labors_by_campaign(campaign_id, start_date=None, end_date=None, 
        ...
        en las parcelas de una campania específica.
        
        Args:
            campaign_id: ID de la campania
        ...
        try:
            ...
            return {
                'error': 'campania no encontrada',
                'campaign_id': campaign_id
            }

        # Obtener parcelas de la campania
    ...
    def get_production_by_campaign(campaign_id):
        ...
        Genera un reporte completo de la producción obtenida en una campania,
    ...
    def get_production_by_plot(plot_id, campaign_id=None, start_date=None, end_date=None):
        ...
        de filtrar por campania y rango de fechas.
        
        Args:
            plot_id: ID de la parcela
            campaign_id: ID de la campania (opcional)
            start_date: Fecha de inicio del filtro (opcional)
            end_date: Fecha de fin del filtro (opcional)
        ...
        # Filtrar por campania si se especifica
        if campaign_id:
            try:
                campaign = Campaign.objects.get(id=campaign_id)
                # Verificar que la parcela pertenezca a la campania
                if not campaign.parcelas.filter(parcela=parcela).exists():
                    return {
                        'error': 'La parcela no pertenece a la campania especificada',
                        'plot_id': plot_id,
                        'campaign_id': campaign_id
                    }
            except Campaign.DoesNotExist:
        ...
        # Histórico por campania (si hay múltiples campanias)
        historico_campanias = []
        campanias_parcela = CampaignPlot.objects.filter(
            parcela=parcela
        ).select_related('campaign').order_by('-campaign__fecha_inicio')

        for cp in campanias_parcela:
            cosechas_campania = Cosecha.objects.filter(
                ciclo_cultivo__cultivo__parcela=parcela,
            ...
            produccion_campania = cosechas_campania.aggregate(
                total=Sum('cantidad_cosechada')
            )['total'] or 0

            historico_campanias.append({
                ...
                'produccion_total': float(produccion_campania),
                'numero_cosechas': cosechas_campania.count(),
        ...
        return {
            ...
            'historico_campanias': historico_campanias,
